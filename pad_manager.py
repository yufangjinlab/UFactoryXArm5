...
class pad_manager:
    ...
    def __init__(self, width, length, angle, movement):
        ...
        self.lego_heights = np.zeroes([length, width], dtype=int)
    ...
    # Width is considered to be the short side of the standard pad (8 studs)
    ...
    def move_pad_y(self, amount):
        x_movement = amount * math.sin(self.angle) * 5 # 5 is for millimeters between studs. may need to be made more precise
        y_movement = amount * math.cos(self.angle) * 5
    ...
    def move_pad_x(self, amount):
        x_movement = amount * math.sin(90-self.angle) * 5
        y_movement = -amount * math.cos(90-self.angle) * 5
        self.movement.move_wherever(x_movement, y_movement, 0, 0, 0, 0)

    # Coordinates are based on the center of the pad
    def set_coordinates(self, x_coord, y_coord):
        ...
        pad_height = self.get_potential_location_height(x_coord, y_coord)
        if  pad_height >= 0:
    ...
    def set_height_values(self, x_coord, y_coord, pad_height):
        ...
        # An exception is caught if the cooridnates go out of bounds
            ...
            for j in range(self.current_lego_length):
                try:
                    if(self.current_lego_orientation == "short-ways"):
                        ...
                        if self.lego_heights[x_coord-lego_length_offset+j, y_coord-lego_width_offset+i] > test_height:
                            test_height = self.lego_heights[x_coord-1+j, y_coord-2+i]
                            test_height_area = 0
                        elif self.lego_heights[x_coord-lego_length_offset+j, y_coord-lego_width_offset+i] == test_height:
                            test_height_area = test_height_area + 1
                except:
                    self.movement.pprint("The coordinates provided are invalid")
                    return -1
    ...
    def place_lego(self, pad_height):
        ...
        height_added_to_z = 21 + (pad_height - 1) * 9.6  # was 18 and 9.6mm
