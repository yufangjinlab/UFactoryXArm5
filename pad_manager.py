...
class PadManager:
    ...
    def __init__(self, width, length, angle, movement):
        ...
        self.lego_heights = np.zeros([length, width], dtype=int)
            ...
            for j in range(self.current_lego_length):
                try:
                    if self.current_lego_orientation == "short-ways":
                        self.lego_heights[
                            x_coord - lego_width_offset + i, y_coord - lego_length_offset + j] = pad_height + 1
                    else:
                        self.lego_heights[
                            x_coord - lego_length_offset + j, y_coord - lego_width_offset + i] = pad_height + 1
                except:
                ...
                try:
                    if self.current_lego_orientation == "short-ways":
                        if self.lego_heights[
                            x_coord - lego_width_offset + i, y_coord - lego_length_offset + j] > test_height:
