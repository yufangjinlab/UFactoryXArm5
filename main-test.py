...
from vision_utils import COLOR_RANGES, get_mask_and_contours, detect_blue_board, board_roi_mask, filter_contours_by_sv
...
class RobotMain(object):
    ...
    def get_all_color_contours(self, hsv_image):
        ...
        for color_name, ranges in COLOR_RANGES.items():
            ...
            #NEW: Will help to filter all colors that are colliding with the wood board
            contours = filter_contours_by_sv(contours, hsv_image, min_area=500, min_s=120, min_v=90)
            color_contours[color_name] = contours
    ...
    def run_precise(self, cap, target_color):
        ...
        #NEW and additional parameters so this won't go crazy
        iteration_count = 0
        max_iterations = 20
        ...
        while self.is_alive:
            ...
            if iteration_count > max_iterations:
                Movement.pprint(f"Max iterations reached, close enough!")
                cv2.destroyWindow("Camera Frame Precision")
                return #NEW: I think we needed to return this function
        ...
        while self.is_alive:
            ...
            # NEW: testing a ROI filtering
            board_box, _ = detect_blue_board(hsv)
            if board_box is not None:
                roi_mask = board_roi_mask(frame.shape, board_box)
                hsv_roi = cv2.bitwise_and(hsv, hsv, mask=roi_mask)
            else:
                hsv_roi = hsv

            # OLD: needs to include the (hsv_roi)
            # OLD: color_contours = self.get_all_color_contours(hsv)

            # NEW: calling out the hsv_roi
            color_contours = self.get_all_color_contours(hsv_roi)
            ...
            for target_color, contours in color_contours.items():
                if contours:
                    for contour in contours:
                        ...
                        #NEW: this is a better threshold
